...
/**
 * Owns {@link PlayerData} lifecycle and stamps every grace-window event the
 * {@link com.koalaguard.manager.SafetyManager} relies on. Also enforces bans
 * at login.
 */
public final class StateListener implements Listener {
    ...
    public StateListener(KoalaGuard plugin) {
        this.plugin = plugin;
    }
    ...
    public void onWorldChange(PlayerChangedWorldEvent event) {
        PlayerData d = plugin.getDataManager().get(event.getPlayer());
        if (d != null) d.lastWorldChangeMs = System.currentTimeMillis();
    }
    ...
    public void onDamage(EntityDamageEvent event) {
        ...
        if (d != null) d.lastDamageMs = System.currentTimeMillis();
    }
